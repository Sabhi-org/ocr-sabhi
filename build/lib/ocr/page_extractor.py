...
from ocr.processors import Resizer, OtsuThresholder, FastDenoiser
...
class PageExtractor:
    def __init__(
        ...
        corner_detector=HoughLineCornerDetector(
            rho_acc=1, theta_acc=180, thresh=100, output_process=True
        ),
    ...
    def __call__(self, image_path, debug=False):
        ...
        if debug:
            cv2.imshow("self._processed", self._processed)
            cv2.waitKey(0)
    ...
    def _extract_page(self):
        ...
        pts = np.array(
            [(x, y)
             for intersection in self._intersections for x, y in intersection]
        )
